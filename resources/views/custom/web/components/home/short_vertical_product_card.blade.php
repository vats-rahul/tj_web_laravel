...
<div class="tallyprime-col2 swiper_product_card slider-ga-event pdp_4el_swiper top-seel-12"
    ...
    <div class="{{ isset($source) && $source == 'mobile' ? 'col-md-12' : 'col-md-3' }}">
        <div class="tallyprime tallyprime-2 tallyprime-2-img">
            @if (isset($product_details['product_label']) && $product_details['product_label'])
                    ...
                    @php
                        $labelColors = json_decode(config('constants.LABEL_COLORS'), true);
                    @endphp

                    @if (isset($labelColors[$product_label]))
                        <div class="sponsored1 shine-{{ $labelColors[$product_label] }}">
                            <span class="{{ $labelColors[$product_label] }}">{{ $product_label }}</span>
                        </div>
                    @endif
            ...
            @php
                $prod_img = !empty($product_details['image']) ? config('constants.DIR_FS_PRODUCT_IMAGE') . $product_details['image'] : config('constants.DIR_FS_PRODUCT_NOIMAGE');
            @endphp
            ...
            <div class="col-md-12">
                    ...
                    <div class="col-md-8 height-90">
                        <div class="accounting-pl1">
                            ...
                            <p class="brand-sel-list">
                                <span>By</span>
                                <a data-gacat="{{ ucfirst($page_type) }} Page"
                                    data-gaaction="Interacted with {{ $ga_action }}"
                                    data-galabel="Clicked on Product Brand | {{ $product_details['product_name'] }} | {{ $page }}"
                                    class="trigger_event" href="{{ url('brand/'.$product_details['brand_slug']) }}">
            ...
            <div class="col-md-12 card-labels">
                @if ($product_details['total_reviews'] > 0)
                        <div class="star-dv1-top">
                            <a class="rating_val trigger_event" href="{{ url('reviews/' . $product_details['slug']) }}"
                                target="_blank" style="color: #fff" data-gacat="{{ ucfirst($page_type) }} Page"
                                data-gaaction="Interacted with {{ $ga_action }}"
                ...
                @else
                        <div class="no-star-dv1-top">
                            <a href="{{ url('add_review/' . $product_details['slug']) }}"
                                class="pc-write-review trigger_event" data-gacat="{{ ucfirst($page_type) }} Page"
                                data-gaaction="Interacted with {{ $ga_action }}"
                                data-galabel="Write a Review | {{ $product_details['product_name'] }} | {{ $page }}"
                                contenteditable="false" target="_blank" title="Write a Review">Add Review
                            </a>
                        </div>
                @endif
                ...
                @if ($hasPrice)
                    <div class="plans-dv">
                        <a href="{{ url("detail/" . $product_details['slug'] . "/#pricing") }}"
                                target="_blank" class="pc-view-plan trigger_event" data-gacat="{{ ucfirst($page_type) }} Page"
                                data-gaaction="Interacted with {{ $ga_action }}"
                                data-galabel="Plans | {{ $product_details['product_name'] }} | {{ $page }}">Plans</a>
                    </div>
                @endif

                @if (!empty($feature) && is_array($feature) && count($feature) > 0)
            ...
            <div class="col-md-12 selling-get-free-demo">
                @if ($hasPrice)
                    ...
                    <div class="get-buy-btn">
                            @if(empty($product_details['vendor_id']) || (!empty($product_details['vendor_id']) && $product_details['vendor_mode'] > 1))
                                <button name="Buy Now" class="tj-loader tj-buynow-btn trigger_event" data-ss="small"
                                    data-proid="{{ $product_details['product_id'] }}" data-slug="{{ $product_details['slug'] }}"
                                    data-gacat="{{ ucfirst($page_type) }} Page" data-gaaction="Interacted with {{ $ga_action }}"
                                    data-galabel="Buy Now | {{ $product_details['product_name'] }} | {{ $page }}"
                                    data-redirect="false">Buy Now</button>
                            @else
                                <button class="tj-acd-form trigger_event" data-proid="{{ $product_details['product_id'] }}"
                                    data-slug="{{ $product_details['slug'] }}" data-gacat="{{ ucfirst($page_type) }} Page"
                                    data-gaaction="Interacted with {{ $ga_action }}"
                                    data-galabel="show Price | {{ $product_details['product_name'] }} | {{ $page }}"
                                    data-proid="{{ $product_details['product_id'] }}" data-action="ShowPrice" data-redirect="false"
                                    data-userintent="Price" style="padding: 12px 0px;">Show Price</button>
                @endif
                    </div>
                @else
                        <div class="get-price">
                        ...
                        <div class="get-buy-btn">
                            <button class="tj-acd-form tj-loader trigger_event"
                                data-proid="{{ $product_details['product_id'] }}" data-slug="{{ $product_details['slug'] }}"
                                data-gacat="{{ ucfirst($page_type) }} Page" data-gaaction="Interacted with {{ $ga_action }}"
