...
tj.acdPopupTemplate.download_product = {
    change_first_form_content: function (action, product_id) {
        tj.acdPopupTemplate.action = action;
        document.getElementById('download-tj-skip-button').innerHTML = 'Skip & Download';
        tj.acdPopupVariables.schedule_button_text = 'Download';
        ...
        if (tj.acdPopupVariables['targetId'] === 'PersonalizedSoftwareRecommendation') {
            ...
            document.getElementById('tj-first-msg-first') && (document.getElementById('tj-first-msg-first').innerHTML = 'You are just a step away');
        }
    ...
    renderAcdQuestions: function (data, containerId) {
        ...
        for (let i in acd_questions) {
            ...
            let optionHtml = '';
            let inputType = 'radio';
            let onChange = 'onChange="tj.acdPopupTemplate.download_product.clickOnAcdContinue();"'
            let optionStyle = '';
            let userAnsBoxStyle = '';
        ...
        tj.acdPopupTemplate.download_product.bindAcdQuestionsEvents(data);
    },
    bindAcdQuestionsEvents: function (data) {
        ...
        $("#acd_ques_continue_btn").off('click').on('click', function (event) {
            ...
            let jsonData = tj.acdPopupTemplate.download_product.validateAcdQuestions([acd_ques[acdQuestionCounter]]);
            ...
            if (!acd_ques[acdQuestionCounter]) {
                tj.acdPopupTemplate.download_product.showScreen(event, 'tj-popup-second');
                return;
            }
    ...
    initFormActions: function () {
        ...
        tj.acdPopupTemplate.download_product.bindEvents();
        ...
        if (tj.acdPopupVariables['targetId'] === 'PersonalizedSoftwareRecommendation') {
            ...
            $('#download-tj-skip-button').hide();
            $('#tj-continue-button').css('width', '100%');
            document.getElementById('tj-popup-third-alternate') && (document.getElementById('tj-popup-third-alternate').style.display = 'none');
        } else {
            document.getElementById('tj-popup-third-alternate') && (document.getElementById('tj-popup-third-alternate').style.display = 'none');
            if (tj.acdPopupVariables['device_type'] != 'desktop') {
                $('#download-tj-skip-button').show();
    ...
    schedule_acd: function (e, jsonData, action) {
        ...
        fetch(tj.acdPopupVariables['tjUrl'][up.en] + '/schedule-acd', l)
            ...
            .then((e) => {
                    var downlodData = {
                        ...
                        name: e.data.name,
                        email: e.data.email,
                        phone: e.data.contact_number,
                        action: tj.acdPopupVariables['downloadBtnText']
                    };
                    1 == e.status
                        ? ( 
                            tj.acdPopupVariables['targetUserIntent'] == 'Download' ? openPdf() :
                            (tj.acdPopupTemplate.download_product.startDownloadSoftware(downlodData)),
                            (tj.acdPopupVariables.schedule_acd_res_data = e.data),
                                    ...
                                    tj.acdPopupTemplate.download_product.change_form_content(e.data))
                            : ($('#tj-acd-sub-button').html(''),
                                    tj.acdPopupVariables['targetId'] !== 'PersonalizedSoftwareRecommendation'
    ...
    change_form_content: function (data) {
        ...
        document.getElementById('tj-third-msg-first').innerHTML =
                "We have a team of software experts who can help you with the right software";
        document
                .getElementById('recommended-products')
                .setAttribute('data-last_acd_time', data.acd_date);
        tj.acdPopupTemplate.download_product.submit_generic_acd_questions();
        ...
        if (data.status) {
            ...
            setTimeout(function () {
                tj.acdPopupTemplate[tj.acdPopupVariables['loadedTemplate']].renderSlider(
                        {'element': '.download_product_popup_slider',
                            'slidesToShow': 1,
                            'slidesToScroll': 1,
                            'responsive': false,
                            'dots': false,
                            'arrows': true,
                            'prevArrow': '<button class="wht-pl1" aria-label="Previous"><img alt="previous" src="' + tj.acdPopupVariables['tjUrl'][up.en] + '/assets/images/V2_img/ar-r1.png" alt="image-error"></button>',
                            'nextArrow': '<button class="wht-pl2" aria-label="next"><img alt="next" src="' + tj.acdPopupVariables['tjUrl'][up.en] + '/assets/images/V2_img/ar-r2.png" alt="image-error"></button>'
                        });
            }, 50);
        } else {
            document.getElementById('recommended-products').innerHTML = '';
    ...
    startDownloadSoftware: function (form) {
        return tj.api.AjaxRequest('POST ', tj.baseUrl + 'techjockey/download_software', function (s) {
            if (s.success) {
                window.open(s.url, '_blank');
            }
        }, onError, onComplete, form);

    }
};

tj.acdPopupTemplate.download_product.renderSlider = function (sliderParam = {}) {
...
tj.acdPopupTemplate.download_product.bindEvents = function () {
    ...
    $('body').on('click', '#download-tj-skip-button', function (e) {
        $("#download-tj-skip-button").unbind();
        ...
        if (all_acd_questions.length) {
            ...
            $(this).attr("id", "download-tj-skip-button-disabled");
            tj.acdPopupTemplate[tj.acdPopupVariables['loadedTemplate']].renderAcdQuestions(
                    all_acd_questions,
                    'acd_second_form_ques_container'
                    );
            ...
            tj.acdPopupTemplate.download_product.showScreen(e, 'tj-popup-second');
        }
