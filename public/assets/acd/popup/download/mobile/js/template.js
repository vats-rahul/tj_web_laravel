...
tj.acdPopupTemplate.download = {
    change_first_form_content: function (action, product_id) {
        ...
        if(tj.acdPopupVariables['videoCampaign'] && Object.keys(tj.acdPopupVariables['videoCampaign']).length) {
            document.getElementById('tj-first-msg-first-parent') && 
            (document.getElementById('tj-first-msg-first-parent').style = 'display:none');
            document.getElementById('campaign-description') && 
            (document.getElementById('campaign-description').innerHTML = tj.acdPopupVariables['videoCampaign'].heading );
        ...
        document.getElementById('tj-acd-sub-button').innerHTML = 'Download';
    ...
    renderAcdQuestions: function (data, containerId) {
        ...
        tj.acdPopupTemplate.download.bindAcdQuestionsEvents();
    },
    ...
    initFormActions: function () {
        ...
        tj.acdPopupTemplate.download.bindEvents();
        ...
        if (tj.acdPopupVariables['targetId'] === 'PersonalizedSoftwareRecommendation') {
            ...
            track_event_new(
                    'Category Page',
                    'Quiz Module',
                    'Clicked on Take a Quiz | '+tj.acdPopupVariables['targetCategoryName']
                    );
                    ...
                    1 == e.status
                            ? (
                                ...
                                (tj.acdPopupVariables.schedule_acd_res_data = e.data),
                                    ...
                                    $('#tj-acd-sub-button').text('Download'),
                                    tj.acdPopupTemplate.download.change_form_content(e.data))
                            : ($('#tj-acd-sub-button').html(''),
                                    tj.acdPopupVariables['targetId'] !== 'PersonalizedSoftwareRecommendation'
                                    ? $('#tj-acd-sub-button').text('Download')
                                    ...
                                    (tj.page_type == "compare" ?
                                        ...
                                        'You have already Downloaded the Free Comparison'): 
                                        (document.getElementById('tj-acd-notification').innerHTML =
                                        'You have already Downloaded the Buyers Guide')),
    ...
    change_form_content: function (data) {
        if (!tj.acdPopupVariables['isEventTriggered']) {
            ...
            tj.events.ga4Events('Lead submitted',{
                ...
                module: (tj.acdPopupVariables['targetFormModule'] && tj.acdPopupVariables['targetFormModule'] != '') ? tj.acdPopupVariables['targetFormModule'] : tj.acdPopupVariables['loadedTemplateName'],
        ...
        if (data.hasOwnProperty('sub_action')) {
            ...
            if (dial_code === '91' && data.sub_action === 'send_otp') {
                ...
                if(tj.page_type == "compare") {
                    (document.getElementById('tj-second-msg-first').innerHTML = 'Your Free Comparison is Downloaded');
                    (document.getElementById('tj-second-msg-second').innerHTML = 'Your Free Comparison is Downloaded');
                }
                return false;
            }
        } else {
            ...
            if (tj.acdPopupVariables['targetId'] !== 'PersonalizedSoftwareRecommendation') {
                ...
                if (search_val != '' && item != null && ($('#tj-acd-search-div').css('display') != "none")) {
                    ...
                    if(tj.page_type == "compare") {
                        get_recommended_products('product', tj.acdPopupVariables['targetProductId']);
                    }else{
                        get_recommended_products('category', tj.acdPopupVariables['targetCategoryId']);
                    }                
                }
                ...
                tj.acdPopupTemplate.download.submit_generic_acd_questions(null);
            }
        }
                ...
                .then((e) => {
                    1 == e.status
                            ? ((document.getElementById('tj-popup-second').style.display = 'none'),
                                ...
                                (tj.page_type == 'compare' ? (document.getElementById('re-p2') && (document.getElementById('re-p2').innerHTML = 'Your Free Comparison is Downloaded')) : ''),
                                $('#tj-first-msg-first').hide(),
...
tj.acdPopupTemplate.download.bindEvents = function () {
                    ...
                    if (e.status) {
                        ...
                        if (tj.acdPopupVariables['device_type'] == 'desktop') {
                            error_html = `<div class="col-md-2">
                                        <div class="recommended-md-4">
                                            <img class="reject-img" alt="image-error" src="${
                                    tj.acdPopupVariables['tjUrl'][up.en] +
...
function downloadPDF(){
    if(tj.page_type == "compare") {
        ...
        tj.loadScript('tj-jspdf','https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js')
        .then(() => {      
            tj.loadScript('tj-html2pdf','https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js')
                .then(() => {      

                    var options = {
                        margin : [0,1],
                        filename: tj.page+'.pdf',
                        image: { type: 'jpeg', quality: 0.98 },
                        html2canvas: { scale: 1.5},
                        jsPDF: { unit: 'in', format: [15,24], orientation: 'portrait' },
                    };

                    let url = window.location.pathname;
                    let segments = url.split('/');
                    let compare_slug = segments[segments.length - 1];
                    $.ajax({
                        url: tj.baseUrl + 'download-compare-pdf',
                        type: "post",
                        dataType: "html",
                        data:{
                            data: JSON.stringify(["compare", compare_slug]),                            },
        ...
        let url = tj.baseUrl + 'assets/pdf/'+cat.slug+'.pdf';
        const a = $("<a>")
            .attr("href", url)
            .attr("download", cat.category_name+".pdf")
            .attr("target", "_self"); 
